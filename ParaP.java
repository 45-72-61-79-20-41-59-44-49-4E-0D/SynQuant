...
public class ParaP {
	...
	public double[][] InitialMu(){
		...
		MuLut1 mu1 = new MuLut1();
		MuLut2 mu2 = new MuLut2();
		for(int i=0;i<50;i++)
			for(int j=0;j<100;j++)
				matrix[i][j] = mu1.mu[i][j];
		for(int i=50;i<100;i++)
			for(int j=0;j<100;j++)
				matrix[i][j] = mu2.mu[i-50][j];
        return matrix;                             
	...
	public double[][] InitialSigma(){
		double [][] matrix = new double[100][100];
		SigmaLut1 sigma1 = new SigmaLut1();
		SigmaLut2 sigma2 = new SigmaLut2();
		for(int i=0;i<50;i++)
			for(int j=0;j<100;j++)
				matrix[i][j] = sigma1.Sigma[i][j];
		for(int i=50;i<100;i++)
			for(int j=0;j<100;j++)

 ...
 *
 * @author: Simon Horne
 * slight changed by Congchao Wang for his application
 */
...
//if (i==32- kernelWidth + 1 && j==100- kernelHeight + 1) System.out.println("Convolve2D: "+output[i][j]);
  ...
  public static double [][] convolution2DPadded(double [][] input,
						...
						int kernelWidth,
						int kernelHeight){
	  double[][] newinput = new double[width + kernelWidth - 1][height + kernelHeight - 1];
	    int top = kernelHeight/2;
	    int left = kernelWidth/2;
	    for(int j=0;j<width ;++j){
	        for(int i=0;i<height;++i){
	        	newinput[j+left][i+top] = input[j][i];
	        }
	      }
    double small [][] = new double [width][height];
    small = convolution2D(newinput,width+ kernelWidth - 1,height + kernelHeight - 1,
